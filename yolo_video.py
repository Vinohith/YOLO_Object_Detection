...
cfg_path = 'yolo2/yolov3.cfg'
weights_path = 'yolo2/yolov3.weights'
labels_path = 'yolo2/coco.names'

LABELS = open(labels_path).read().strip().split('\n')
COLORS = np.random.uniform(0, 255, size=(len(LABELS), 3))

# image = cv2.imread('/home/vinohith/YOLO/00049.png')
# (h, w) = image.shape[0:2]
# processed_image = cv2.dnn.blobFromImage(image, 1./255, (416,416), swapRB=True, crop=False)
video = cv2.VideoCapture('/home/vinohith/SegNet/Video 2.mp4')
...
while True:
	_, frame = video.read()
	(h, w) = frame.shape[0:2]
	processed_image = cv2.dnn.blobFromImage(frame, 1./255, (416,416), swapRB=True, crop=False)
